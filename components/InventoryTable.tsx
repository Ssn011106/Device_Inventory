...
const InventoryTable: React.FC<InventoryTableProps> = ({ devices, role, fields, onEdit, onDelete, onViewHistory }) => {
  ...
  // Prioritize critical columns for cleaner view, but allow all
  const criticalFields = ['assetTag', 'equipmentDescription', 'partNumber', 'serialNumber', 'status', 'currentOwner', 'location'];
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left min-w-[2000px]">
          <thead className="bg-slate-50/50 border-b border-slate-100">
            <tr>
              {fields.map(field => (
                <th key={field.id} className="px-6 py-6 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">
                  {field.label}
                </th>
              ))}
              <th className="px-6 py-6 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] text-right sticky right-0 bg-slate-50/90 backdrop-blur-sm shadow-[-10px_0_15px_-3px_rgba(0,0,0,0.05)]">Actions</th>
            </tr>
          </thead>
            ...
            {!devices || devices.length === 0 ? (
              <tr>
                <td colSpan={fields.length + 1} className="px-6 py-32 text-center">
                   <div className="flex flex-col items-center">
                    ...
                    <p className="text-slate-400 font-bold text-lg">Your inventory pool is currently empty.</p>
                    <p className="text-slate-300 text-sm font-medium mt-1">Import your 235 assets to populate this table.</p>
                   </div>
                </td>
              </tr>
            ) : (
              devices.map((device) => (
                <tr key={device.id} className="hover:bg-slate-50/30 transition-colors group">
                    ...
                    return (
                      <td key={field.id} className="px-6 py-5">
                        {field.id === 'status' ? (
                          ...
                          <span className="font-mono text-[10px] bg-slate-900 px-4 py-2 rounded-xl text-white font-black border border-slate-800 shadow-lg block w-max min-w-[80px] text-center">
                            {value || 'N/A'}
                          </span>
                        ) : (
                  ...
                  <td className="px-6 py-5 text-right whitespace-nowrap sticky right-0 bg-white group-hover:bg-slate-50 transition-colors shadow-[-10px_0_15px_-3px_rgba(0,0,0,0.05)]">
                    <div className="flex items-center justify-end space-x-2">
                      <button onClick={() => onEdit(device)} className="text-indigo-600 hover:bg-indigo-50 px-5 py-2.5 rounded-2xl transition-all text-[10px] font-black uppercase tracking-widest">
                        {isAdmin ? 'Edit' : 'View'}
                      </button>
